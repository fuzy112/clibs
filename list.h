...
/* This file was ported from Linux kernel. */
#ifndef LIST_H
#define LIST_H
...
#define LIST_WRITE_ONCE(variable, value)                                      \
  do                                                                          \
    {                                                                         \
      __typeof__ ((variable)) _value = (value);                               \
      __atomic_store (&(variable), &_value, __ATOMIC_RELAXED);                \
    }                                                                         \
  while (0)

#define LIST_READ_ONCE(variable)                                              \
  __atomic_load_n (&(variable), __ATOMIC_RELAXED)

#define list_smp_store_release(ptr, value)                                    \
  __atomic_store_n ((ptr), (value), __ATOMIC_RELEASE)

#define list_smp_load_acquire(ptr) __atomic_load_n ((ptr), __ATOMIC_ACQUIRE)
...
{
  LIST_WRITE_ONCE (list->next, list);
  list->prev = list;
}

static inline bool
__list_add_valid (struct list_head *new_, struct list_head *prev,
 ...
 */
static inline void
__list_add (struct list_head *new_, struct list_head *prev,
            struct list_head *next)
{
  if (!__list_add_valid (new_, prev, next))
    return;

  next->prev = new_;
  new_->next = next;
  new_->prev = prev;
  LIST_WRITE_ONCE (prev->next, new_);
}
...
{
  next->prev = prev;
  LIST_WRITE_ONCE (prev->next, next);
}
 ...
 * LIST_WRITE_ONCE() overhead of a regular list_del_init(). The code that uses
 * this needs to check the node 'prev' pointer instead of calling list_empty().
 */
static inline void
...
/**
 * list_replace_init - replace old entry by new_ one and initialize the old one
 * @old : the element to be replaced
 * @new_ : the new_ element to insert
 *
 * If @old was empty, it will be overwritten.
...
{
  return LIST_READ_ONCE (head->next) == head;
}
...
list_del_init_careful (struct list_head *entry)
{
  __list_del_entry (entry);
  entry->prev = entry;
  list_smp_store_release (&entry->next, entry);
}
...
{
  struct list_head *next = list_smp_load_acquire (&head->next);
...
#define list_first_entry_or_null(ptr, type, member)                           \
  ({                                                                          \
    ...
    struct list_head *pos__ = LIST_READ_ONCE (head__->next);                  \
...
__hlist_del (struct hlist_node *n)
{
  ...
  LIST_WRITE_ONCE (*pprev, next);
  if (next)
    LIST_WRITE_ONCE (next->pprev, pprev);
}
...
/**
 * hlist_del_init - Delete the specified hlist_node from its list and
 * initialize
 * @n: Node to delete.
 *
...
/**
 * hlist_add_head - add a new_ entry at the beginning of the hlist
 * @n: new_ entry to be added
...
hlist_add_head (struct hlist_node *n, struct hlist_head *h)
{
  ...
  LIST_WRITE_ONCE (n->next, first);
  if (first)
    LIST_WRITE_ONCE (first->pprev, &n->next);
  LIST_WRITE_ONCE (h->first, n);
  LIST_WRITE_ONCE (n->pprev, &h->first);
}

/**
 * hlist_add_before - add a new_ entry before the one specified
 * @n: new_ entry to be added
...
{
  LIST_WRITE_ONCE (n->pprev, next->pprev);
  LIST_WRITE_ONCE (n->next, next);
  LIST_WRITE_ONCE (next->pprev, &n->next);
  LIST_WRITE_ONCE (*(n->pprev), n);
}

/**
 * hlist_add_behind - add a new_ entry after the one specified
 * @n: new_ entry to be added
...
{
  LIST_WRITE_ONCE (n->next, prev->next);
  LIST_WRITE_ONCE (prev->next, n);
  LIST_WRITE_ONCE (n->pprev, &prev->next);

  if (n->next)
    LIST_WRITE_ONCE (n->next->pprev, &n->next);
}
