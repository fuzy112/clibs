...
static void
rb_insert_node (struct rb_tree_node *node, struct rb_root *tree)
{
...
static int
rb_get_label (const void *node, char *label)
{
...
main ()
{
...

  {
    ...
    rb_for_each_safe (iter, n, &tree)
      {
        struct rb_tree_node *node = rb_entry (iter, struct rb_tree_node, node);
        rb_erase (iter, &tree);
        free (node);
      }
  }
